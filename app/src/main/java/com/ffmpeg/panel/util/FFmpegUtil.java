...
public class FFmpegUtil {
    ...
    public static String[] transformHLAudio(String inputPath, String outputPath) {
        //-ab 64k 设置音频码率
        //-ac 1 设置声道数为1 mono（默认不写）  立体声 2 strteo
        //-ar 44100 设置采样率为44100Hz
        String transformAudioCmd = "ffmpeg -i %s -vcodec copy -acodec aac -ar 44100 -ab 128k -ac 1 -y %s";
        transformAudioCmd = String.format(transformAudioCmd, inputPath, outputPath);
        return transformAudioCmd.split(" ");
    }

    public static String[] videoMerge(String inputPath, String outputPath) {

        String transformAudioCmd = "ffmpeg -safe 0 -y -f concat -i %s -c copy %s";
        transformAudioCmd = String.format(transformAudioCmd, inputPath, outputPath);
        return transformAudioCmd.split(" ");
    }
    ...
    public static String[] extractVideo(String inputPath, String outputPath) {
        //-an: disable audio
        String extractVideoCmd = "ffmpeg -i %s -vcodec copy -an -y %s";
